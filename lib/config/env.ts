...
const envSchema = z.object({
  ...
  AI_DEDUP_ENABLED: booleanFlag,
  GEMINI_DEDUP_MODEL: z.string().trim().min(1).catch('gemini-3.1-flash-lite'),
...
export function getEnv(): Env {
  if (!cached) {
    cached = envSchema.parse({
      ...
      AI_DEDUP_ENABLED: process.env.AI_DEDUP_ENABLED,
      GEMINI_DEDUP_MODEL: process.env.GEMINI_DEDUP_MODEL,
